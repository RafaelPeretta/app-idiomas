...
public class FirestorePhaseLoader : MonoBehaviour
{
    ...
    private async void LoadPhaseFromFirestore(string documentID)
    {
        try
        {
            ...
            // 🔥 ADIÇÃO AQUI – AGORA O ID DA AVALIAÇÃO É SALVO
            PhaseDataLoad loadedPhase = new PhaseDataLoad
            {
                id = documentID,  // <-- Correção fundamental
            ...
            if (PhaseManager.Instance != null)
            {
                ...
                Debug.Log($"[FirestorePhaseLoader] Fase carregada ({loadedQuestions.Count} questões). ID = {documentID}");
